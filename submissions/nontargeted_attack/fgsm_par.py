...
def proc_gpu_task(task_list, task_q, fin_q, verbose=False):
    ...
    for task_id in iter(task_q.get, -1):
        ...
        if task[2] == 0:
            ...
            K.clear_session()
            # NOTE must close session, otherwise old ops
            # get kept, causing problem when restoring from .pb
            K.get_session().close()
            grad_dict = {}
...
#                (('inceptionv3adv', 2), ('incresv2ensadv', 2), step_size*0.1, noise_size*0.5), \
#                (('inceptionv3adv', 2), ('incresv2ensadv', 2), step_size*0.1, noise_size*0.5), \
#                (('incresv2ensadv', 1), ('incresv2ensadv', 1), step_size*0.1, noise_size*0.5), \
#                (('inceptionv3adv', 2), ('incresv2ensadv', 2), step_size*0.1, noise_size*0.5)), \
            ]
        ...
        if phase[0][0] == 'RST':
            ...
            if i_phase:
                task = ('phase {0} reload {1}'.format(i_phase, phase[0]), (), 5, \
                        (phase[0], use_avg_pred, is_targeted))
                task_list.append(task)
    ...
    #    print('Phase', p)
    #    print('Loading task', v[0])
    #    print('Task set', v[1])
    #    print('First tasks', v[2])
    ...
    print('Total images:', len(images_info))
    print('augmentation (craft):', grad_aug_scale)
